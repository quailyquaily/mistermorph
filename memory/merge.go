...
func MergeLongTerm(existing LongTermContent, draft PromoteDraft) LongTermContent {
	incomingGoals := normalizeKVItems(draft.GoalsProjects)
	incomingFacts := normalizeKVItems(draft.KeyFacts)
	return LongTermContent{
		Goals: mergeKV(existing.Goals, incomingGoals),
		Facts: mergeKV(existing.Facts, incomingFacts),
	}
}
