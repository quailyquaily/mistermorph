...
func (s *TerminalRenderer) Render(newbuf *RenderBuffer) {
	...
	// In inline mode the physical cursor is still relative to the old frame.
	// Reach its origin before shrinking the buffer: move otherwise clamps the
	// remembered cursor row to the new height and leaves old rows on screen.
	if s.clear && !s.flags.Contains(tFullscreen) && newHeight < curHeight {
		s.move(newbuf, 0, 0)
	}
